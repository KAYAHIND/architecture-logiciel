...
router.post('/', async (req, res) => {
  try {
    ...
    if (!produit || !quantite || !client) return res.status(400).json({ erreur: 'Champs manquants' });
...
router.get('/', async (req, res) => {
  try {
    ...
    if (cached) return res.json({ source: '⚡ CACHE Redis', traitee_par: INSTANCE, commandes: JSON.parse(cached) });
